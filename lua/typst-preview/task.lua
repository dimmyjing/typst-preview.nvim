...
function exec_cmd(self, command, arguments, err_callback, result_callback)
  utils.debug(
...
function PreviewTask:spawn(on_error, on_link_set)
  ...
  if config.opts.extra_args ~= nil then
    ...
    if type(extra) == 'function' then
      local ok, res = pcall(extra, self.path, self.mode, config.opts.port)
  ...
  exec_cmd(self, 'tinymist.doStartPreview', { args }, function(err)
    ...
    -- also test with next tinymist release, the respective code has been completely refactored
...
-- server-side close
function PreviewTask:subscribe(event, handler) end
...
function PreviewTask:scroll_to(filepath, line, character)
  exec_cmd(self, 'tinymist.scrollPreview', {
    self.task_id,
    {
      event = 'panelScrollTo',
      filepath = filepath,
